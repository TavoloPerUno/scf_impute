...
class Knn_Imputer:
    ...
    def _fit(self, X, column, k=10, is_categorical=False, np_mean_mode=np.asarray([0])):
    ...
    def _transform(self, X, column, clf, is_categorical, np_mean_mode=np.asarray([0])):
    ...
    def knn(self, X, column, k=10, is_categorical=False, np_mean_mode=np.asarray([0])):
        ...
        clf = self._fit(X, column, k, is_categorical, np_mean_mode)
        if clf is None:
            return X
        else:
            X_imputed = self._transform(X, column, clf, is_categorical, np_mean_mode)
            return X_imputed
